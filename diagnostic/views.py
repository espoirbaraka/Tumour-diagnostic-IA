...
def cancer_result(request):
    if request.method=='POST':
        ...
        if answer is not None:
            ...
            max_prob = answer.values[max_prob_index]
            if(max_state==0):
                res=f"Tu as une faible probabilité d'avoir le cancer metastatique. La probabilité de ne pas l'avoir est de {max_prob*100} %"
            elif(max_state==1):
                res=f"Tu as une probalibilte d'avoir le cancer metastatique. La probabilité de l'avoir est de {max_prob*100} %"
        else:
             print("Erreur de retour")
        ...
        template = 'cancer_result.html'
        return render(request, template, context)

def calcium_result(request):
    if request.method=='POST':
        try:
            coma=int(request.POST.get('coma'))
            headache=int(request.POST.get('tete'))
        ...
        if answer is not None:
            max_prob_index = answer.values.argmax()  # Récupérer l'indice de la probabilité maximale
            max_state = answer.state_names['Serum'][max_prob_index]  # Récupérer l'état correspondant
            max_prob = answer.values[max_prob_index]
            if(max_state==0):
                res=f"Tu as une faible probabilité d'augmentation du calcium sérique. La probabilité de ne pas l'avoir est de {max_prob*100} %"
            elif(max_state==1):
                res=f"Tu as une probalibilte d'augmentation du calcium sérique. La probabilité de l'avoir est de {max_prob*100} %"
        else:
             print("Erreur de retour")
...
def headache_result(request):
    if request.method=='POST':
        ...
        if answer is not None:
            max_prob_index = answer.values.argmax()  # Récupérer l'indice de la probabilité maximale
            max_state = answer.state_names['Headache'][max_prob_index]  # Récupérer l'état correspondant
            max_prob = answer.values[max_prob_index]
            if(max_state==0):
                res=f"Tu as une faible probabilité d'avoir le maut de tete. La probabilité de ne pas l'avoir est de {max_prob*100} %"
            elif(max_state==1):
                res=f"Tu as une probalibilte d'avoir le maut de tete. La probabilité de l'avoir est de {max_prob*100} %"
        else:
             print("Erreur de retour")
